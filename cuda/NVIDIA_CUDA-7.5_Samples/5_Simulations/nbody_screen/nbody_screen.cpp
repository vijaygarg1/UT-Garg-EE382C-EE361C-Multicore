...
void initGL(int *argc, char **argv)
{
    ...
    for (dispno_it = 0; dispno_it < displayCount; dispno_it++)
    {
        ...
        screen_get_display_property_iv(screenDisplayHandle[dispno_it], SCREEN_PROPERTY_ATTACHED, &active);
        if (active)
        {
            if (dispno == dispno_it)
            {
                // Map the window buffer to user requested display port
                screen_set_window_property_pv(screen_window, SCREEN_PROPERTY_DISPLAY, (void **)&screenDisplayHandle[dispno]);
                break;
            }
        }
    }
    ...
    if (bFullscreen)
    {
        ...
        int windowSize[2];
        if (screen_get_window_property_iv(screen_window, SCREEN_PROPERTY_SIZE, windowSize))
        {
    ...
    eglQueryContext(eglDisplay, eglContext, EGL_CONTEXT_CLIENT_TYPE, &contextRendererType);
...
showHelp()
{
    ...
    printf("\t-width=<w>        (where w represents the width of the window to open)\n");
    printf("\t-width=<h>        (where h represents the height of the window to open)\n");
